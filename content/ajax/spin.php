...
if (empty($_GET['_unique']) || mysqli_num_rows(mysqli_query($link, "SELECT `id` FROM `players` WHERE `hash`='".prot($_GET['_unique'])."' LIMIT 1 FOR UPDATE"))==0) exit();

$reservedBalance=mysqli_fetch_array(mysqli_query($link, "SELECT SUM(`balance`) AS `sum` FROM `players`"));
$reservedWaitingBalance=mysqli_fetch_array(mysqli_query($link, "SELECT SUM(`amount`) AS `sum` FROM `deposits`"));
...
$player=mysqli_fetch_array(mysqli_query($link, "SELECT * FROM `players` WHERE `hash`='".prot($_GET['_unique'])."' LIMIT 1"));
...
$colick=(max($player['client_seed'],($player['client_seed']*-1)))*9;
...
mysqli_query($link, "UPDATE `players` SET `balance`=TRUNCATE($newBalance,10),`t_spins`=`t_spins`+1 WHERE `id`=$player[id] LIMIT 1");
mysqli_query($link, "INSERT INTO `spins` (`wager`,`multiplier`,`player`) VALUES ($wager,$multiplier,$player[id])");
...
$c_seed=max($c_seed,($c_seed*-1));


mysqli_query($link, "UPDATE `players` SET `server_seed`='$s_seed',`client_seed`='$c_seed',`old_server_seed`='$player[server_seed]',`old_client_seed`='$player[client_seed]' WHERE `id`=$player[id] LIMIT 1");


echo json_encode(array('spin'=>'spin','error'=>'no','data'=>$multiplier,'spinTo'=>$spinTo,'reduceTo'=>$balance,'tspins'=>($player['t_spins']+1),'tspins_'=>mysqli_num_rows(mysqli_query($link, "SELECT `id` FROM `spins`"))));
