...
switch ($_GET['con']) {
  case 'recent_spins':
    $content.='<table id="stats-table" class="container">';
    $content.='<tr>';
    $content.='<th>ID</th>';
    $content.='<th>Alias</th>';
    $content.='<th>Bet</th>';
    ...
    $content.='<th>Time</th>';
    $content.='</tr>';
    $query=mysqli_query($link, "SELECT * FROM `spins` ORDER BY `time` DESC LIMIT 30");
    $sl=false;
    while ($row=mysqli_fetch_array($query)) {
      if (mysqli_num_rows(mysqli_query($link, "SELECT `alias` FROM `players` WHERE `id`=$row[player] LIMIT 1"))!=0) {
        $player_inf_=mysqli_fetch_array(mysqli_query($link, "SELECT `alias` FROM `players` WHERE `id`=$row[player] LIMIT 1"));
  ...
  case 'leaderbord':
    ...
    $content.='<th>Time</th>';
    $content.='</tr>';
    $query=mysqli_query($link, "SELECT *,(`wager`*`multiplier`) AS `return` FROM `spins` ORDER BY `return` DESC LIMIT 30");
    $sl=false;
    while ($row=mysqli_fetch_array($query)) {
      if (mysqli_num_rows(mysqli_query($link, "SELECT `alias` FROM `players` WHERE `id`=$row[player] LIMIT 1"))!=0) {
        $player_inf_=mysqli_fetch_array(mysqli_query($link, "SELECT `alias` FROM `players` WHERE `id`=$row[player] LIMIT 1"));
  ...
  case 'fair':
    $unique=$_GET['_unique'];
    $player=mysqli_num_rows(mysqli_query($link, "SELECT `id` FROM `players` WHERE `hash`='".prot($unique)."' LIMIT 1"));
    if ($player!=0) $player=mysqli_fetch_array(mysqli_query($link, "SELECT `id`,`server_seed`,`client_seed`,`old_server_seed`,`old_client_seed` FROM `players` WHERE `hash`='".prot($unique)."' LIMIT 1"));
    else exit();
